 ...
 *
 */
public class ConectaK extends EstadoJuego<ConectaK>{
	...
	public double distCentro() {
		return Math.abs((ultimoMov.c()+1)-((double)(tablero.nColumnas()+1)/(double) 2))+(Math.abs(ultimoMov.f()+1)-((double)(tablero.nFilas()+1)/(double)2));
	}
	static class ordenCentro implements Comparator<ConectaK>{

		@Override
		public int compare(ConectaK o1, ConectaK o2) {
			if(o1.distCentro()<o2.distCentro())return -1;
			else if(o1.distCentro()>o2.distCentro())return 1;
			else return 0;
		}
